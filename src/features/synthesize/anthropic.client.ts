 ...
 *
 * SDK의 오류 클래스는 `name`을 따로 세우지 않아 전부 `'Error'`로 온다. 연결
 * 실패와 취소는 `status`도 없어서, 구조만 보는 `errors.ts`가 둘을 구분하지
 * 못한다. 클래스를 아는 곳은 SDK를 import한 이 파일뿐이므로 여기서 이름을
 * 세운다. `instanceof`는 번들 후 클래스 이름이 뭉개져도 그대로 동작한다.
 *
 * 이 번역이 실제 SDK 오류와 맞는지는 `tests/unit/synthesize/anthropic.client.test.ts`가
 * 진짜 응답으로 확인한다.
 */
function tagError(error: unknown): unknown {
