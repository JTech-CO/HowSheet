...
- 현재 phase: P8 - 프롬프트 품질 평가셋
- 상태: DONE
- 마지막 갱신: 2026-09-15 KST

## 직전에 끝낸 것

**P8 - 프롬프트 품질 평가셋**

- `pnpm eval:prompts` - 한 명령으로 돌고 `artifacts/eval/`에 결과를 남긴다. 12초
- `src/features/evaluate/` - 기계적 채점기. 검사 8종, 판정은 통과·실패 둘뿐
- 자료 5종 × 요소 조합 13종 × 디자인 135종 + 잘림 5 = 조립기 프롬프트 8,780건 전부 통과
- 좋은 프롬프트에 변이를 가한 카나리아 81종을 매번 돌린다. 하나라도 빠져나가면 실패
- `docs/HowSheet_v2_평가_AI채점_가이드.md` - AI 채점을 붙이는 조건. 구현하지 않았다
- 테스트 +303 (단위 667, 통합 83). 음성 검증 18건 전부 물었다

**설계 판단**

- **채점은 기계적 검사만 한다.** 사용자 확정. AI 채점은 붙일 자리만 가이드로 남겼다
- **카나리아를 평가 명령에서 매번 돌린다.** 단위 테스트에만 두면 평가 명령은 채점기가
  눈을 감아도 통과한다
- **부정은 서술어가 정한다.** 절 끝의 부정 서술어와 낱말 바로 곁의 부정만 본다. 적팀과
  청팀이 이 선을 양쪽에서 밀어 정했다
- **숫자는 단위와 함께 맞춘다.** 규칙 문구의 "## 3."이 "노드 3대"를 허락하지 않게
- **평가 스크립트는 키를 읽지 않는다.** 실제 AI 출력은 `prompts/captured/`에 넣을 자리만 둔다

**이번 phase에서 드러난 것**

- **생성 프롬프트가 스스로 모순이었다.** '혼합' 타이포의 "이 셋을 넘지 않는다"와 금지
  목록의 "셋 이상 섞지 않는다". 사용자 확정으로 코드용 고정폭을 세지 않게 고쳤다
- **첫 채점기는 적팀에게 210건을 내줬고, 조인 채점기는 청팀의 좋은 프롬프트 35건 중
  33건을 떨어뜨렸다.** 고친 뒤 적팀 194건 적발, 청팀 1건(옳은 적발)
- **한국어로만 쓴 꾸민 사실은 기계로 잡지 못한다.** 남은 우회의 대부분이고 AI 채점
  가이드가 맡는다

## 다음 할 일

하네스에 P9가 없다. 아래에서 고른다.

1. **실제 AI 출력을 평가셋에 넣는다.** `prompts/captured/`가 비어 있어 실제 Haiku
   출력은 한 건도 채점하지 않았다. 절차는 AI 채점 가이드 §7 (사용자 작업)
2. **CI 워크플로를 v2 게이트에 맞춘다.** `ci.yml`이 v1의 없는 스크립트를 불러 첫 단계에서
   멈춘다. `pnpm eval:prompts`도 여기에 넣는다
3. **SDK 동적 import가 단위 테스트 첫 실행을 5초 넘게 만든다.** P8 회귀에서 한 번 실패했다
4. 미결: 제품정의 §3의 "모델 선택" UI를 어느 phase에 둘지
5. 미결: `@testing-library/jest-dom`을 setup에 연결할지 의존성에서 뺄지
6. 미결: AI 채점을 붙일지. 붙인다면 평가용 키를 스크립트가 읽는 경계를 정해야 한다 (가이드 §5)
