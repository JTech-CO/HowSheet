...
- 현재 phase: 출시 점검 (하네스 밖, P8 이후)
- 상태: 출시 보류 - 배포처 결정과 실제 API 확인 대기
- 마지막 갱신: 2026-09-16 KST

## 직전에 끝낸 것

**출시 점검** (`artifacts/qa/phase-reports/release-check-2026-09-16.md`)

- 판정: **아직 출시하지 않는다.** 차단 요소 7건 중 5건을 고쳤고 2건이 남았다
- 감사 8개 차원 + 반박 검증 + 완결성 비평가 → 결함 80건. 사실이 아닌 것 0건
- 고친 뒤 커밋별 반박 검토 5개를 다시 돌려 지적을 반영했다
- 테스트 단위 667 → 688, 통합 83 → 102, e2e 33 → 36. 음성 검증 15건 전부 물었다

**고친 것**

- **미리보기 크래시** - 깊은 인용 6KB 한 줄이 `RangeError`로 앱 전체를 백지로
  만들었다. 앱 계층 오류 경계, 재귀 한계만 안내로 바꾸는 미리보기, 살균기가
  돌 수 없으면 던지는 검사(DOMPurify는 조용히 입력을 돌려준다)
- **잘린 AI 프롬프트** - 출력 예산을 초안에서 뽑고 모델 한계 안에 둔다.
  `stop_reason`은 허용 목록으로 판정한다
- **사실과 다른 안내** - 자료 전송 고지, 잘림 사전 안내, v1 메타데이터, README.
  설정 화면과 합성 경로가 **같은 키 저장소**를 쓰게 했다
- **CI·줄바꿈** - v2 게이트 11종, `.gitattributes`, 조립기의 줄바꿈 정규화
- **헤더의 저장 경고** - 본문으로 옮기고 320px 메모리 모드 e2e를 더했다

**드러난 것**

- **검토가 첫 고침의 구멍을 여럿 찾았다.** 메모리 모드에서 "새로고침하면 돌아옵니다"라고
  말하는 오류 화면, `max_tokens`만 막는 판정, 테스트 쪽에서 가린 CRLF 어긋남
- **공개 URL이 빈 화면이다.** Pages가 main 브랜치 원본을 게시하고 있다

## 다음 할 일

1. **배포처를 정한다** (차단, 사용자 결정). 전용 출처가 필요하다. 그때까지 GitHub
   Pages 게시를 꺼 두기를 권한다
2. **전용 키로 실제 API를 한 번 돌린다** (차단, 사용자 작업). 짧은 자료와 3만 자 이상
   자료를 각각. 잘림 판정과 요금을 확인한다
3. 다크 테마 기본 버튼 대비 2.54:1 (major)
4. 생성 실패 이유가 스크린 리더에 읽히지 않는다 (major, P7 DoD 4)
5. 큰 표·깊은 인용 미리보기가 메인 스레드를 수 초 멈춘다 (major)
6. 저장소 열기 거부 시 "불러오는 중입니다"에서 멈출 수 있다 (major)
7. 실제 AI 출력을 평가셋에 넣는다 (`prompts/captured/`, 사용자 작업)
8. SDK 동적 import가 단위 테스트 첫 실행을 5초 넘게 만든다
9. 빈 게이트 정리: `eslint.config.js`의 `src/reader-runtime/**` 블록, 부르는 곳 없는
   `test:coverage`, `.gitignore`의 없는 스크립트 안내
...
- ~~P8 평가셋의 자동화 범위가 정해지지 않았다~~ **(2026-09-15 사용자 확정)**: 기계적
  검사만 한다. AI 채점은 `docs/HowSheet_v2_평가_AI채점_가이드.md`에 조건만 적었다
- **AI 채점을 붙이려면 평가용 키의 경계를 정해야 한다.** 키를 만지는 모듈은 하나라는
  P4 DoD 5와 부딪힌다. 가이드 §5에 제안을 적었다
...
- **배포처가 정해지지 않았다 (출시 차단).** GitHub Pages 기본 도메인은 계정의 다른
  Pages 사이트 118개와 출처를 공유해, 그중 어느 스크립트든 localStorage의 키를 읽는다.
  제품정의와 하네스에 배포 항목이 없다. `release-build.yml`은 임시로 산출물만 만든다
- **시스템 프롬프트의 "자료 원문을 그대로 인용해 남긴다"가 제품정의 §6.2에 없다.**
  이 요구 때문에 AI 출력이 초안보다 길고 출력 예산이 커진다. 인용을 빼고 앱이 자료를
  붙이면 출력과 비용이 크게 준다. 제품 결정이다
- **제품정의 머리가 "초안. 사용자 승인 전"이고 §11 네 항목이 미체크다.**
  출시 전에 승인 여부를 정해야 한다
...
| 2026-09-15 | 프롬프트 채점은 기계적 검사만 한다. AI 채점은 가이드만 남긴다                        | 사용자 확정                                                                                                                                                                                                       | `src/features/evaluate/`, `docs/HowSheet_v2_평가_AI채점_가이드.md`        | 사용자               |
| 2026-09-15 | 코드용 고정폭은 폰트 계열 수에 넣지 않는다                                           | '혼합' 타이포와 금지 목록이 같은 프롬프트에서 폰트 계열 수를 다르게 말했다                                                                                                                                        | `src/domain/prompt.rules.ts`, `spec.types.ts`, 제품정의 §5.1              | 사용자               |
| 2026-09-15 | 채점기를 `src/features/evaluate/`에 두고 실행기는 Node가 .ts를 직접 돈다             | 도메인 규칙·조립기를 import하고 타입 검사·경계 규칙을 그대로 받는다. 번들에는 들어가지 않는다. 새 도구를 들이지 않는다                                                                                            | `scripts/eval-prompts.ts`                                                 | 에이전트 (P8)        |
| 2026-09-15 | 카나리아를 파일이 아니라 변이로 만들고 평가 명령에서 매번 돌린다                     | 파일은 조립기 문구가 바뀌면 엉뚱한 이유로 떨어진다. 단위 테스트에만 두면 평가 명령이 눈 감은 채점기로도 통과한다                                                                                                  | `src/features/evaluate/canaries.ts`                                       | 에이전트 (P8)        |
| 2026-09-15 | 디자인 135종을 고르지 않고 전부 돈다                                                 | 폰트 모순이 '혼합' × '기술' 조합에서만 나왔다. 전부 돌아도 12초다                                                                                                                                                 | `src/features/evaluate/cases.ts`                                          | 에이전트 (P8)        |
| 2026-09-15 | 부정을 절 끝 서술어와 낱말 곁에서만 판정한다                                         | 절 어디에든 부정어가 있으면 부정으로 보던 첫 판이 완곡 표현에 210건 뚫렸고, 절 끝만 보던 둘째 판이 좋은 프롬프트 33건을 떨어뜨렸다                                                                                | `src/features/evaluate/text.ts`                                           | 에이전트 (P8)        |
| 2026-09-15 | 숫자를 단위와 함께 대조하고 규칙 문구는 단위까지 같아야 근거로 친다                  | 규칙 문구의 "## 3."이 "노드 3대"를 통과시켰다                                                                                                                                                                     | `src/features/evaluate/facts.ts`                                          | 에이전트 (P8)        |
| 2026-09-15 | 평가 스크립트가 키를 읽지 않고 실제 AI 출력은 `captured/`로 받는다                   | 키를 만지는 모듈은 하나다 (INV-01, P4 DoD 5)                                                                                                                                                                      | `tests/fixtures/eval/prompts/`                                            | 에이전트 (P8)        |
| 2026-09-15 | 평가 자료를 포맷터에서 빼고 결과 폴더를 gitignore한다                                | 재작성 프롬프트가 자료를 바이트 단위로 인용해 표 정렬이 바뀌면 인용이 깨진다. 결과는 매번 새로 쓴다                                                                                                               | `.prettierignore`, `.gitignore`                                           | 에이전트 (P8)        |
| 2026-09-16 | 사용자가 출시 점검 차단 요소 2~6번과 문구 수정을 진행하기로 했다                     | 1번(배포처)은 결정, 7번(실제 API)은 사용자 작업이라 뺐다                                                                                                                                                          | `artifacts/qa/phase-reports/release-check-2026-09-16.md`                  | 사용자               |
| 2026-09-16 | 오류 경계를 `components/ui`가 아니라 `src/app/`에 둔다                               | 저장 모드를 알아야 사실을 말한다. 메모리 모드에서 "새로고침하면 돌아옵니다"는 자료를 잃게 한다 (AGENTS §4 뒷문장)                                                                                                 | `src/app/AppErrorBoundary.tsx`                                            | 에이전트 (출시 점검) |
| 2026-09-16 | 출력 예산을 초안 × 1.6 + 1,000으로, 바닥 8,000·천장 60,000·모델 한계 안에서 뽑는다   | 고정 8,000은 인용 요구 때문에 긴 자료에서 반드시 잘렸다. `max_tokens`는 상한이라 짧게 끝나면 비용이 늘지 않는다                                                                                                   | `src/features/synthesize/model.ts`                                        | 에이전트 (출시 점검) |
| 2026-09-16 | `stop_reason`은 `end_turn`만 완성본으로 친다                                         | `max_tokens`만 막던 판이 `refusal`·`model_context_window_exceeded`를 통과시켰다. 새 이유가 생겨도 기본이 막음이어야 한다                                                                                          | `src/features/synthesize/anthropic.client.ts`                             | 에이전트 (출시 점검) |
| 2026-09-16 | 앱은 키 저장소 인스턴스를 하나만 쓴다 (`sharedApiKeyStore`)                          | 둘이면 메모리 모드·다른 탭·저장 실패 뒤 화면 문구와 실제 전송이 어긋난다                                                                                                                                          | `src/storage/api-key.store.ts`, `settings.store.ts`, `synthesize/deps.ts` | 에이전트 (출시 점검) |
| 2026-09-16 | DOMPurify가 돌 수 없는 환경이면 살균기가 던진다                                      | DOMPurify는 이때 입력을 그대로 돌려준다. 실패를 닫는다 (INV-05)                                                                                                                                                   | `src/features/sanitize/sanitize-html.ts`                                  | 에이전트 (출시 점검) |
| 2026-09-16 | 조립기가 줄바꿈을 LF로 맞춘 뒤 센다. 테스트에서 맞추지 않는다                        | 테스트 쪽 정규화는 제품의 어긋남을 가리고 감지기를 없앴다                                                                                                                                                         | `src/features/compose/compose.ts`                                         | 에이전트 (출시 점검) |
| 2026-09-16 | `.gitattributes`로 줄바꿈을 LF에 고정한다                                            | 윈도우 새 클론에서 `format:check` 172개 파일이 실패했다                                                                                                                                                           | `.gitattributes`                                                          | 에이전트 (출시 점검) |
| 2026-09-16 | **(사용자 확인 대기)** `deploy.yml`을 `release-build.yml`로 바꾸고 업로드하지 않는다 | 배포처가 출처를 공유한다(미결 질문). 한 번 수동 실행으로 공유 출처에 올라가는 길을 닫았다. 대상이 정해지면 업로드 단계를 붙인다                                                                                   | `.github/workflows/release-build.yml`                                     | 에이전트 (출시 점검) |
| 2026-09-16 | Vite `base`를 `./`로 둔다                                                            | 라우터가 없어 절대 base의 근거(v1 중첩 경로)가 사라졌다. 끝에 슬래시가 있는 어느 경로에서도 같은 산출물이 동작한다                                                                                                | `vite.config.ts`                                                          | 에이전트 (출시 점검) |
| 2026-09-16 | e2e 웹서버 예산을 240초로 올린다                                                     | `pnpm build`가 typecheck까지 돌아 차가운 CI 러너에서 120초가 빠듯하다                                                                                                                                             | `playwright.config.ts`                                                    | 에이전트 (출시 점검) |
...
| 2026-09-15 | `pnpm eval:prompts`                                                     | 성공 - 생성 8780/8780, 재작성 5/5, 카나리아 81/81종 (바탕 15개), 12.4초                                       | `artifacts/qa/phase-reports/P8.md`                       |
| 2026-09-15 | 적대 검증 1차 적팀 5명                                                  | 후보 263건 중 210건 우회 → 고친 뒤 194건 적발, 16건은 뜻의 문제로 남김                                        | `artifacts/qa/phase-reports/P8.md`                       |
| 2026-09-15 | 청팀 3명 (좋은 프롬프트 35건)                                           | 조인 채점기가 33건 오탐 → 고친 뒤 1건 (자료에 없는 합계, 옳은 적발)                                           | `artifacts/qa/phase-reports/P8.md`                       |
| 2026-09-15 | P8 음성 검증 18건                                                       | 18건 모두 의도대로 실패. 카나리아 미적용은 단위 테스트가 못 잡고 실행기의 공허 방지만 잡는다                  | `artifacts/qa/phase-reports/P8.md`                       |
| 2026-09-15 | 전체 회귀 (format/lint/typecheck/test/verify*/build)                    | 성공 - 단위 667, 통합 83. 단위 첫 실행에서 SDK import 시간 초과 1건, 재실행 두 번 667/667                     | `artifacts/qa/phase-reports/P8.md`                       |
| 2026-09-16 | 출시 점검 게이트 11종 (작업 트리)                                       | 성공 - 게이트 전부 통과                                                                                       | `artifacts/qa/phase-reports/release-check-2026-09-16.md` |
| 2026-09-16 | 윈도우 새 클론에서 게이트 11종 (점검 전 e14992f)                        | 실패 - `format:check` 172개 파일, 단위 1건 (CRLF)                                                             | `artifacts/qa/phase-reports/release-check-2026-09-16.md` |
| 2026-09-16 | 실제 빌드 브라우저 점검                                                 | 외부 요청은 키 있을 때 api.anthropic.com 1건뿐. 깊은 인용 미리보기가 앱을 백지로. 메모리 모드 경고 44×1,079px | `artifacts/qa/phase-reports/release-check-2026-09-16.md` |
| 2026-09-16 | 공개 Pages URL                                                          | 빈 화면 - 원본 index.html 게시, `/src/main.tsx` 404                                                           | `artifacts/qa/phase-reports/release-check-2026-09-16.md` |
| 2026-09-16 | 감사 8차원 + 반박 검증 + 비평가                                         | 결함 80건, 사실 아님 0건. blocker 7                                                                           | `artifacts/qa/phase-reports/release-check-2026-09-16.md` |
| 2026-09-16 | 고침 뒤 게이트 11종                                                     | 성공 - 단위 688, 통합 102, e2e 36, 평가 8780/8780, 번들 gzip 138.76KB                                         | `artifacts/qa/phase-reports/release-check-2026-09-16.md` |
| 2026-09-16 | 고침 음성 검증 15건                                                     | 15건 모두 의도대로 실패                                                                                       | `artifacts/qa/phase-reports/release-check-2026-09-16.md` |
| 2026-09-16 | 윈도우 새 클론 e1632c3에서 게이트 11종                                  | 성공 - CRLF 0개, 단위 688, 통합 102, e2e 36. 커밋 14개 각각 typecheck·lint·unit·integration 통과              | `artifacts/qa/phase-reports/release-check-2026-09-16.md` |
| 2026-09-16 | GitHub Actions CI (e1632c3)                                             | 성공 - v2 이후 처음. verify 62초, E2E 3종 131초                                                               | GitHub Actions run 35084339978                           |
