...
- `components/studio/PromptResult/` - 복사, `.md` 내려받기, 회차 버튼으로
  이전 결과 보기
- `store/generate.store.ts` - 결과를 **덮지 않고 쌓는다.** 회차를 붙이고
  상한 10개를 둔다
- `utils/filename.ts` - v2 프롬프트 파일명 규칙. v1의 `.howsheet.json` 경로는 지웠다
- 테스트 447개(단위 364, 통합 83). 음성 검증 13건 전부 물었다

**설계 판단**

- **재생성은 덮지 않고 쌓는다.** DoD 3의 "확인을 받거나 남긴다" 중 남기는 쪽이다.
  확인 대화상자는 한 번 더 누르게 하면서 되돌리지는 못한다
- **회차는 스토어가 붙인다.** 파일명이 회차를 쓰는데 "몇 번째로 만들었는가"는
  합성기가 아니라 목록을 든 쪽이 안다. 상한에 걸려 오래된 것이 빠져도 번호를
  다시 쓰지 않는다 - 같은 이름의 파일이 두 번 내려가면 구분할 수 없다
- **파일명은 자료의 첫 줄에서 만든다.** v2에는 제목 필드가 없다. 규칙(금지 문자,
  예약 이름, 코드 포인트 80자)은 v1에서 그대로 가져왔다
- **복사 폴백은 이미 있던 것을 쓴다.** `utils/clipboard.ts`와 `CopyButton`이
  P0에서 살아남아 있었고 필요한 동작이 그대로였다
- **DoD 4는 두 층으로 본다.** 통합 테스트가 줄 접기 선언이 있는지 보고, 실제
  값은 브라우저에서 320px로 쟀다. 테스트만으로 "가로 스크롤 없음"을 주장하지 않는다

**이번 phase에서 드러난 것**

- **`utils/filename.ts`에 소비자도 테스트도 없었다.** v1에서 넘어온 뒤로 아무도
  부르지 않아 규칙이 한 번도 검증된 적이 없었다. 단위 테스트 16개를 붙였고
  예약 이름·코드 포인트 절단·제어 문자 순서가 음성 검증에서 물었다
- **실제 브라우저에서 복사가 폴백 경로로 갔다.** 미리보기 창이 클립보드 권한을
  주지 않아, 확인하기 어려운 쪽이 우연히 확인됐다
- **P5의 통합 테스트가 스토어 모양 변경을 그대로 통과했다.** `result` 하나에서
  `results` 목록으로 바뀌었는데 16개가 다 살았다. 화면을 통해 단언한 덕이다

## 다음 할 일

1. **P7 - 접근성·모바일·인쇄**
   - 키보드로 흐름을 완주할 수 있는지 실기기에서 본다 (INV-09)
   - 320px 터치 대상 44px 이상 (INV-10). 결과 상자는 P6에서 이미 320px를 봤다
   - 인쇄 경로와 `@testing-library/jest-dom` 연결 여부를 함께 정한다
   - 번들 크기(현재 gzip 183KB)와 SDK 동적 import
2. **P8** - 프롬프트 품질 평가셋. 자동화 범위가 아직 미결이다
3. 미결: 제품정의 §3의 "모델 선택" UI를 어느 phase에 둘지
