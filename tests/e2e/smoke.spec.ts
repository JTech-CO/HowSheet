...
/**
 * 하네스 M1 DoD 4 — 최소 앱이 `/`에서 렌더링되고 브라우저 콘솔 error가 0건이다.
...
test.describe('M1 스모크', () => {
  test('대시보드가 / 에서 렌더링되고 콘솔 error가 없다', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: 'HowSheet' })).toBeVisible();
    await expect(page.getByTestId('scaffold-note')).toBeVisible();
